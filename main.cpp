...
void DrawInWindow(sf::RenderWindow& window, bool matrix[SIZE][SIZE], int width, int height, int cell_size, int cell_grind, int ui_height, bool is_play) {
    ...
    if (is_play) {
        text.setString(
                "Space - pause/play   Delete - clear   F/G - chill    <-/-> - speed    LBM - draw / RBM - erase   process - play");
    } else {
        text.setString(
                "Space - pause/play   Delete - clear   F/G - chill    <-/-> - speed    LBM - draw / RBM - erase   process - pause");
    }
